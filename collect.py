# zbieranie wlasnych probek landmarkow - personalizacja modelu pod konkretna dlon, dopelnienie datasetu od Kaggle
import csv
...
# wspolrzedne czytane przez training.py
LM_COLUMNS = [f"0_point_lm_{i}_{axis}" for i in range(21) for axis in ("x", "y", "z")]
# litery bazowe ktore zna model
VALID = set("abcdefghiklmnoprstuwyz")
# dwuznaki o wlasnym, charakterystycznym ksztalcie
EXTRA_LABELS = {"1": "sz"}
# diakrytyki/dwuznaki bez wlasnej klasy = baza + ruch
DIACRITIC_HINT = "ą=a ę=e ć=c ł=l ń=n ó=o ś=s  ch/cz=h  j=i  ż/ź=z   sz=klawisz 1"

def load_existing():
    # wczytuje dotychczasowe probki
    rows = []
    if OUT_CSV.exists():
...
def main():
    ...
    while True:
        ...
        if not has_hand:
            cv2.putText(frame, "brak dloni", (20, 120), cv2.FONT_HERSHEY_SIMPLEX,0.8, (0, 165, 255), 2)
        cv2.putText(frame, f"razem: {len(rows)}", (20, frame.shape[0] - 20), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 255, 0), 2)
        cv2.putText(frame, DIACRITIC_HINT, (20, frame.shape[0] - 50), cv2.FONT_HERSHEY_SIMPLEX, 0.55, (180, 180, 180), 1)
